...
def load_rel_ix_dicts():
    ...
    with open(data_path + 'ix_song_keyword.dict', 'rb') as handle:
        song_keyword = pickle.load(handle)
    with open(data_path + 'ix_keyword_song.dict', 'rb') as handle:
        keyword_song = pickle.load(handle)
        
    return song_user, user_song, song_artist, artist_song, song_keyword, keyword_song
...
# 한 user에 대한 path 생성
def find_paths_user_to_songs(start_user, song_artist, artist_song, song_keyword, keyword_song, 
    ...
    while len(stack) > 0:
            ...
            if entity in song_artist:
                ...
                for index in index_list:
                    ...
                    if artist not in front.entities:
                        ...
                        stack.append(new_state)
            if entity in song_keyword:
                keyword_list = song_keyword[entity]
                index_list = get_random_index(sample_nums, len(keyword_list))
                for index in index_list:
                    keyword = keyword_list[index]
                    if keyword not in front.entities:
                        new_path = copy.deepcopy(front.path)
                        new_path[-1][2] = consts.SONG_KEYWORD_REL
                        new_path.append([keyword, consts.KEYWORD_TYPE, consts.END_REL])
                        new_state = PathState(new_path, front.length + 1, front.entities|{keyword})
                        stack.append(new_state)
        ...
        # type이 keyword인 경우
        elif type == consts.KEYWORD_TYPE and entity in keyword_song:
            song_list = keyword_song[entity]
            index_list = get_random_index(sample_nums, len(song_list))
            for index in index_list:
                song = song_list[index]
                if song not in front.entities:
                    new_path = copy.deepcopy(front.path)
                    new_path[-1][2] = consts.KEYWORD_SONG_REL
                    new_path.append([song, consts.SONG_TYPE, consts.END_REL])
                    new_state = PathState(new_path, front.length + 1, front.entities|{song})
                    stack.append(new_state)
...
# path 생성
def load_data(song_artist, artist_song, song_keyword, keyword_song, 
    ...
    for user, pos_songs in tqdm(list(user_song_split.items())) :    # relation 데이터 사용
        ...
        for pos_song in pos_songs :
            ...
            if song_to_paths is None:    # positive song 첫 반복에만 실행
                if version == "train":
                    song_to_paths = find_paths_user_to_songs(user, song_artist, artist_song, song_keyword, keyword_song, 
                                                             song_user_split, user_song_split, 3, len_3_branch)    # train에서는 user_song train 사용
                    song_to_paths_len5 = find_paths_user_to_songs(user, song_artist, artist_song, song_keyword, keyword_song, 
...
def main():
    ...
    song_user, user_song, song_artist, artist_song, song_keyword, keyword_song = load_rel_ix_dicts()
    ...
    if args.train:
        ...
        # path 생성
        load_data(song_artist, artist_song, song_keyword, keyword_song, 

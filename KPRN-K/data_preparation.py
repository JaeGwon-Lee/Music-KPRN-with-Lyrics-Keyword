...
# relation 딕셔너리 생성
def song_data_prep(songs_file, interactions_file, keywords_file, export_dir, user_limit):
    ...
    # song_keyword.dict
    song_keyword = keywords[['song_id', 'keyword']]
    song_keyword_dict = song_keyword.set_index('song_id')['keyword'].to_dict()    # 딕셔너리로 (key : song_id , value : keyword_list)
    with open(export_dir + consts.SONG_KEYWORD_DICT, 'wb') as handle:
        pickle.dump(song_keyword_dict, handle, protocol=pickle.HIGHEST_PROTOCOL)    # 딕셔너리를 이진파일로 저장

    # keyword_song.dict
    keyword_song_dict = {}
    for row in song_keyword.iterrows():
        for keyword in row[1]['keyword']:
            if keyword not in keyword_song_dict:
                keyword_song_dict[keyword] = []
            keyword_song_dict[keyword].append(row[1]['song_id'])
    with open(export_dir + consts.KEYWORD_SONG_DICT, 'wb') as handle:
        pickle.dump(keyword_song_dict, handle, protocol=pickle.HIGHEST_PROTOCOL)
...
def ix_mapping(import_dir, export_dir, mapping_export_dir):
    ...
    with open(import_dir + consts.SONG_KEYWORD_DICT, 'rb') as handle:
        song_keyword = pickle.load(handle)
    with open(import_dir + consts.KEYWORD_SONG_DICT, 'rb') as handle:
        keyword_song = pickle.load(handle)

    songs = set(song_user.keys()) | set(song_artist.keys()) | set(song_keyword.keys())
    users = set(user_song.keys())
    artists = set(artist_song.keys())
    keywords = set(keyword_song.keys())
    pad_token = consts.PAD_TOKEN
    
    # type 임베딩
    type_to_ix = {'user': consts.USER_TYPE, 'song': consts.SONG_TYPE, 'artist': consts.ARTIST_TYPE, 
                  'keyword': consts.KEYWORD_TYPE, pad_token: consts.PAD_TYPE}
    ...
    relation_to_ix = {'song_user': consts.SONG_USER_REL, 'user_song': consts.USER_SONG_REL,
                      ...
                      'song_keyword': consts.SONG_KEYWORD_REL, 'keyword_song': consts.KEYWORD_SONG_REL,
    ...
    entity_to_ix.update({(keyword, consts.KEYWORD_TYPE): ix + len(songs) + len(users) + len(artists) for ix, keyword in enumerate(keywords)})
    ...
    song_keyword_ix = convert_to_ids(entity_to_ix, song_keyword, consts.SONG_TYPE, consts.KEYWORD_TYPE)
    keyword_song_ix = convert_to_ids(entity_to_ix, keyword_song, consts.KEYWORD_TYPE, consts.SONG_TYPE)
    ...
    with open(ix_dir + consts.SONG_KEYWORD_DICT, 'wb') as handle:
        pickle.dump(song_keyword_ix, handle, protocol=pickle.HIGHEST_PROTOCOL)
    with open(ix_dir + consts.KEYWORD_SONG_DICT, 'wb') as handle:
        pickle.dump(keyword_song_ix, handle, protocol=pickle.HIGHEST_PROTOCOL)
...
def main():
    ...
    song_data_prep(consts.DATASET_DIR + args.songs_file,           # ../res/song_meta.json
                   ...
                   consts.KEYWORD_DIR + args.keywords_file,        # ../res/keyword/keyword.json
